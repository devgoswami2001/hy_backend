...
class EmployerIdView(generics.GenericAPIView):  # Changed from CreateAPIView
...
class CompanyPostsByUserCompanyView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # Get posts based on user role and preferences
        ...
        if user.role == 'hr':
            hr_record = getattr(user, 'hr_user', None)
            # HR users might have restrictions on what posts they can see
            # For now, they can see all company posts, but you can add more logic here
            pass
        
        return queryset.order_by('-is_pinned', '-created_at')

    def list(self, request, *args, **kwargs):
        """Override list to provide enhanced response with company info"""
        user = request.user
        employer_profile = self.get_employer_profile(user)
        ...
        if page is not None:
            ...
            # Add company and user info to paginated response
...
class CompanyProfileViewSet(viewsets.ModelViewSet):
    ...
    def check_edit_permission(self, user):
        ...
        if user.role == 'employer':
            return True  # Employers can always edit their profile
        elif user.role == 'hr':
            hr_record = getattr(user, 'hr_user', None)
    ...
    def get_object(self):
        """Get company profile based on user role"""
        employer_profile = self.get_employer_profile(self.request.user)
    ...
    def update(self, request, *args, **kwargs):
        """Update company profile with permission checks"""
        try:
